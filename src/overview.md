...
两张图必须分开，是因为它们的时间单位不同。图 1 的单位是日期：哪一周发热，哪一天换表，哪一段在产后窗口里。图 2 的单位是决策回合：发现一次、确认一次、解释一次、动作一到两个、八到十二周回看一次。把细胞照片贴进图 1 的日历轴，等于用没有日期的“年轻”去抹掉有日期的台阶。把图 2 画成没有回看的箭头，等于把决策回合偷换成流水线。第一部到第五部，其实只是在给这两种单位分别加刻度。

颜色也有约定。图 1 里虚线偏灰，是约束，不是已经失败；实线偏绿，是走出来的速度，不是已经健康；折线偏褐，是事件，不是道德。图 2 里回落用绿，未回落用褐，不是审美，是为了强迫失败进入主图。若有人把褐色方块从宣传页上裁掉，只留上排四个箭头，这本书最硬的一笔就不在了。读图时请先找褐色。找得到，闭环才是可检验的；找不到，流程图无论多精致，都还在卖不能失败的希望。手绘检查还可以加第四项：三问有没有分别落在斜率、薄点和失败分支上。落在购物车里的问，不算落在图上。
